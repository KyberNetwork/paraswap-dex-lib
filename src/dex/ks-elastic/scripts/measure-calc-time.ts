...
const logger = getLogger('KsElasticMeasureScript');
...
const uniV3 = new KsElastic(network, 'KsElastic', dexHelper);
...
const executeOnlySyncOperations = async (states: DeepReadonly<PoolState>[]) => {
  await Promise.all(
    states.map(async state => {
      await ksElasticMath.queryOutputs(state, amounts, zeroForOne, side);
    }),
  );
};
